"""
    test InfoNCE loss changes for per landmark
"""
...
def get_res3(pth, learner):
    tester = Tester(logger, config, retfunc=2, split='Train')
...
if __name__ == '__main__':
    ...
    for pth in pths:
        ...
        epochs = [450]
        for epoch in epochs:
            data_path = f"./tmp/tester_gtv5/ssl_{pth}_gtv_epoch_{epoch}.npy"
            if switch or not os.path.exists(data_path):
                ...
                print(res1)
                np.save(f"./tmp/tester_gtv5/ssl_{pth}_gtv_epoch_{epoch}.npy", res1)
                np.save(f"./tmp/tester_gtv5/ssl_{pth}_count.npy", length)
            else:
                res1 = np.load(data_path)
                # res3 = np.load(data_path[:-5] + "3.npy")
                print(res1)
            # import ipdb; ipdb.set_trace()
            plt.plot(np.arange(0,19), res1, label=f"{epoch} " + pth)
            # plt.plot(np.arange(0,19), res3, label="3 " + pth)

    plt.legend()
    plt.savefig("./tmp/tester_gtv5.png")
    plt.savefig("./tmp/tester_gtv5.pdf")
    ...
    for pth in pths:
        ...
        # for epoch in epochs:
        data_path = f"./tmp/tester_gtv5/ssl_{pth}_gtv.npy"
        if True or not os.path.exists(data_path):
            ...
            res1, length = get_res3(full_path, learner)
            print(res1)
            np.save(f"./tmp/tester_gtv5/ssl_{pth}_gtv.npy", res1)
            np.save(f"./tmp/tester_gtv5/ssl_{pth}_count.npy", length)
        else:
            res1 = np.load(data_path)
            # res3 = np.load(data_path[:-5] + "3.npy")
            print(res1)
        # import ipdb; ipdb.set_trace()
        plt.plot(np.arange(0, 19), res1, label=pth)
        # plt.plot(np.arange(0,19), res3, label="3 " + pth)
