...
class UNet_Pretrained(nn.Module):
    ...
    def get_concat_features(self, x, upsample="nearest", *args, **kwargs):
        feas = self.forward(x, *args, **kwargs)
        new_fea_list = []
        for id_depth, fea in enumerate(feas):
            if len(fea.shape) == 3:
                fea = fea.unsqueeze(0)
            fea_reshaped = torch.nn.functional.upsample( \
                fea, scale_factor=2 ** (5 - id_depth), mode=upsample)
            if len(fea_reshaped.shape) == 3:
                fea_reshaped = fea_reshaped.unsqueeze(0)
            new_fea_list.append(fea_reshaped)
            # import ipdb; ipdb.set_trace()
        return torch.cat(new_fea_list, axis=1)

    def get_scaled_features(self, x, upsample="bilinear"):
        return self.get_concat_features(x, upsample=upsample)


class Wrapper(nn.Module):
    def __init__(self, net, probmap):
...
class Probmap(nn.Module):
    def __init__(self):
        super(Probmap, self).__init__()
        CONF_GRAD = False
        self.conf_5 = nn.Parameter(torch.ones((12, 12))*0.1, requires_grad=CONF_GRAD)
        self.conf_4 = nn.Parameter(torch.ones((24, 24))*0.1, requires_grad=CONF_GRAD)
        self.conf_3 = nn.Parameter(torch.ones((48, 48))*0.1, requires_grad=CONF_GRAD)
        self.conf_2 = nn.Parameter(torch.ones((96, 96))*0.1, requires_grad=CONF_GRAD)
        self.conf_1 = nn.Parameter(torch.ones((192, 192))*0.1, requires_grad=CONF_GRAD)
        self.conf_0 = nn.Parameter(torch.ones((384, 384))*0.1, requires_grad=CONF_GRAD)

class Probmap_np(object):
    def __init__(self, config):
        # super(Probmap, self).__init__()
        CONF_GRAD = False
        self.conf_5 = np.ones((12,12)) * 0.01
        self.conf_4 = np.ones((24,24)) * 0.01
        self.conf_3 = np.ones((48,48)) * 0.01
        self.conf_2 = np.ones((96,96)) * 0.01
        self.conf_1 = np.ones((192,192)) * 0.01
        self.conf_0 = np.ones((384,384)) * 0.01
        self.config = config

    def save(self):
        runs_dir = self.config['runs_dir']
        filename = os.path.join(runs_dir, "prob_params.npy")
        np.save(
            filename,
            {"conf_5": self.conf_5,
             "conf_4": self.conf_4,
...
if __name__ == '__main__':
    model = UNet_Pretrained(3, 16)
    wtf = torch.zeros([1, 3, 224, 224], dtype=torch.float)
    wtf = model.get_concat_features(wtf)
    import ipdb; ipdb.set_trace()
