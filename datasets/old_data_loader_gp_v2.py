...
class Cephalometric(data.Dataset):
    def __init__(self, pathDataset, mode, size=[384, 384], be_consistency=False, patch_size=192, pre_crop=False,
                 rand_psize=False, ref_landmark=None, prob_map=None, retfunc=1, use_prob=True, ret_name=False,
                 num_repeat=10):
    ...
    def select_point_from_prob_map(self, prob_map, size=(192, 192)):
        ...
        chosen_x1 = np.random.randint(int(pad_scale * size_x), int((1-pad_scale) * size_x))
        chosen_x2 = np.random.randint(int(pad_scale * size_x), int((1-pad_scale) * size_x))
        chosen_y1 = np.random.randint(int(pad_scale * size_y), int((1-pad_scale) * size_y))
        chosen_y2 = np.random.randint(int(pad_scale * size_y), int((1-pad_scale) * size_y))
