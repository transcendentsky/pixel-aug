...
class Cephalometric(data.Dataset):
    def __init__(self, pathDataset, mode="Train", size=384, patch_size=192,
        ...
        if use_prob:  # prob map setting
            ...
            if prob_map is None:
                self.prob_map = self.prob_map_from_landmarks(self.size, sharpness=sharpness)
    ...
    def prob_map_from_landmarks(self, size=(384, 384), sharpness=0.2):
        """
        ...
        print("====== Save Prob map to ./tmp")
        cv2.imwrite(f"tmp/prob_map_sharp{sharpness}.jpg", (prob_map * 255).astype(np.uint8))
        return prob_map
    ...
    def retfunc1(self, index):
        ...
        margin_w = left
        margin_h = top
        # print(margin_h, margin_w)
        # if margin_h + patch_size >= self.size[1] or margin_h < 0:
        #     import ipdb; ipdb.set_trace()
        # if margin_w + patch_size >= self.size[0] or margin_w < 0:
        #     import ipdb; ipdb.set_trace()
...
class Test_Cephalometric(data.Dataset):
    def __init__(self, pathDataset='/home1/quanquan/datasets/Cephalometric/', mode="Test1+2", size=[384, 384], return_mode="simple", id_oneshot=1, pre_crop=False):
        ...
        if mode == 'Oneshot':
            ...
            self.pth_Image = os.path.join(*os.path.split(self.pth_Image)[:-1])
            print("Using Pseudo labels: ", self.pth_Image)
            start = 1
            end = 150
        else:
            raise ValueError

        normalize = transforms.Normalize([0], [1]) # 0.5
        transformList = []
    ...
    def ref_pseudo_landmarks(self, index):
        np.random.seed()
        item = self.list[index]
        landmark_list = list()
        with open(os.path.join(self.pth_Image, item['ID'] + '.json'), 'r') as f:
