...
class Cephalometric(data.Dataset):
    def __init__(self, pathDataset, mode="Train", size=384, patch_sizes=[64, 224],
                 pre_crop=False, ref_landmark=None, prob_map=None,
    ...
    def retfunc1(self, index):
        ...
        ps1, ps2 = self.patch_sizes[0], self.patch_sizes[1]
        crop_imgs1, chosen_h1, chosen_w1 = self.get_patch(ps1, raw_w, raw_h, item)
        crop_imgs2, chosen_h2, chosen_w2 = self.get_patch(ps2, raw_w, raw_h, item)

        return {'raw_imgs': item['image'],
                'raw_loc': torch.LongTensor([raw_h, raw_w]),
                'crop_imgs1': crop_imgs1,
                'chosen_loc1': torch.LongTensor([chosen_h1, chosen_w1]),
                'crop_imgs2': crop_imgs2,
                'chosen_loc2': torch.LongTensor([chosen_h2, chosen_w2]),
                'ID': item['ID']}

    def get_patch(self, patch_size, raw_w, raw_h, item):   # get cropped patch
        b1_left = 0
        b1_top = 0
        ...
        return crop_imgs, chosen_h, chosen_w
