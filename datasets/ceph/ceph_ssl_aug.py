"""
    Dataloader with more augmentation ops
        please use with ssl_ensemble.py
    Including:
        AutoContrast
        Contrast
        Equalize
...
class Cephalometric(data.Dataset):
    def __init__(self, pathDataset, mode="Train", size=384, patch_size=192,
        ...
        self.hyper_params_to_record = {"cj_brightness": cj_brightness,
    ...
    def retfunc2(self, index):
        ...
        preprocess_names = ['AutoContrast', 'Contrast', 'Equalize', 'Posterize2', "ShearX", "ShearY"]
        preprocess_level = [0.7,0.7,0.7,1.0, 0.7,0.7]
        pil_img = pil_img.resize((384,384))
        idx = random.choice([0,1,2,3,4,5])
        pil_img = apply_augment(pil_img, name=preprocess_names[idx], level=preprocess_level[idx])

        item['image'] = self.transform2(pil_img)
