...
class Evaluater(object):
    ...
    def record_old(self, pred, landmark):
        ...
        """
        c = pred[0].shape[0]
        assert c == 19 or c == 1, f"Shape error!, Got {pred.shape}"
    ...
    def cal_metrics_per_lm(self):
        temp = np.array(self.RE_list)
        Mean_RE_channel = temp.mean(axis=0)
        mre = Mean_RE_channel.mean()
        ret_dict = {}
        ret_dict['mre'] = mre
        assert Mean_RE_channel.shape[0] == 19 or Mean_RE_channel.shape[0] == 1, f"Got {Mean_RE_channel.shape}"
        for i in range(Mean_RE_channel.shape[0]):
            ret_dict[f'mre_lm_{i}'] = Mean_RE_channel[i]

        sdr_dict = {}
        for radius in self.recall_radius:
            total = temp.size
            shot = (temp < radius).sum() # .sum(axis=0)
            sdr_dict[f"SDR {radius}"] = shot * 100 / total
            # for i in range(mre.shape[0]):
            #     ret_dict[f'sdr_{radius}_lm_{i}'] = sdr_dict[f"SDR {radius}"][i]
...
if __name__ == '__main__':
    # for debug
    pred = np.random.random((19, 2))
    lm = np.random.random((19, 2))
    evaler = Evaluater(logger=None, size=(384,384), original_size=(2400,1800))
    evaler.record_new(pred, lm)
    evaler.record_new(pred, lm)
    evaler.record_new(pred, lm)
    res = evaler.cal_metrics_per_lm()
    print(res)
