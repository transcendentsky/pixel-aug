...
def reg_loss(cos_map, gt_y, gt_x, func):
    # raw_fea_list, tmpl_feature, raw_loc[:, 0], raw_loc[:, 1]
    b, h, w = cos_map.shape
    total_loss = list()
    R_ratio = 0.05
    Radius = int(max(cos_map.shape) * R_ratio)
    offset_x = torch.zeros(2*Radius, 2*Radius, dtype=torch.float)
    offset_y = torch.zeros(2*Radius, 2*Radius, dtype=torch.float)
    ...
    for id in range(b):
        ...
        min_x, max_x = max(gt_x[id] - nearby, 0), min(gt_x[id] + nearby, w)
        chosen_patch = cos_map[id, :, min_y:max_y, min_x:max_x]
        offset_h = cos_map[id, 0, min_y:max_y, min_x:max_x]
        offset_w = cos_map[id, 1, min_y:max_y, min_x:max_x]
...
def _tmp_reg_loss(ii, raw_fea_list, crop_fea_list, raw_loc, chosen_loc):
    scale = 2 ** (5-ii)
    raw_loc, chosen_loc = raw_loc // scale, chosen_loc // scale
    tmpl_feature = torch.stack([crop_fea_list[ii][[id], :, chosen_loc[id][0], chosen_loc[id][1]] \
                                for id in range(raw_loc.shape[0])]).squeeze()
    reg_loss(raw_fea_list, tmpl_feature, raw_loc[:, 0], raw_loc[:, 1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/ssl/std.yaml", help="default configs")
    ...
    dataset = Cephalometric(config['dataset']['pth'], 'Train', patch_size=config_train['patch_size'])
