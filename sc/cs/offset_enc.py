...
#     # print(f"shapes: ", feature.shape, template.shape)
#     feature = feature.permute(0, 2, 3, 1)
#     if len(template.size()) == 1:
...
class Learner(LearnerWrapper):
    def __init__(self, logger, config, *args, **kwargs):
        ...
        self.mask_module = Net()
        self.regx_module = Net()
        self.regy_module = Net()
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        fea, fea_heatmap, fea_regx, fea_regy = self.net(raw_imgs)
        fea2, fea_heatmap2, fea_regx2, fea_regy2 = self.net_patch(raw_imgs)

        vectors = fea_heatmap2[chosen_loc]


        # (fea_heatmap, vector) == mask
        # (fea_regx[p] - fea_regx2[lm]) == offset
        # (fea_regx[p] - fea_regx[lm]) == offset
