...
if __name__ == "__main__":
    ...
    if True:
        print("Load CKPTs")
        ckpt_net = '/home1/quanquan/code/landmark/code/runs/ssl/ssl_cs2/debug7/model_epoch_latest.pth'
        net.load_state_dict(torch.load(ckpt_net))
        ckpt_patch = '/home1/quanquan/code/landmark/code/runs/ssl/ssl_cs2/debug7/model_patch_epoch_latest.pth'
        net_patch.load_state_dict(torch.load(ckpt_patch))
    ...
    for epoch in range(start_epoch, config_train['num_epochs']):
        ...
        for data in metriclogger.log_every(dataloader, print_freq=print_freq, header=""):
            ...
            import ipdb; ipdb.set_trace()
            save_image(heatmap.unsqueeze(1), tfilename(config['base']['runs_dir'], "heatmap_pred.png"))
            save_image(regression_y.unsqueeze(1), tfilename(config['base']['runs_dir'], "regression_y_pred.png"))
            save_image(regression_x.unsqueeze(1), tfilename(config['base']['runs_dir'], "regression_x_pred.png"))
            import ipdb; ipdb.set_trace()
        ...
        if (epoch + 0) % config_train['save_interval'] == 0 and epoch >= 0:
            ...
            csvlogger.record(_d)
            logger.info(f"Epoch: {epoch}; Record: {_d}")
            logger.info(runs_dir + "/model_epoch_latest.pth")
            torch.save(net.state_dict(), runs_dir + "/model_epoch_latest.pth")
            torch.save(net_patch.state_dict(), runs_dir + "/model_patch_epoch_latest.pth")
            # import ipdb; ipdb.set_trace()
    # # Test
    _d = tester.test(net)
    _d['record_time'] = datetime.now()
