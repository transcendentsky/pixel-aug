...
def _tmp_loss_func(ii, raw_fea_list, crop_fea_list, raw_loc, chosen_loc, nearby=None):
    scale = 5 - ii
    gt_y, gt_x = raw_loc[0] // (2 ** scale), raw_loc[1] // (2 ** scale)
    tmpl_y, tmpl_x = chosen_loc[0] // (2 ** scale), chosen_loc[1] // (2 ** scale)
    tmpl_feature = torch.stack([crop_fea_list[ii][[id], :, tmpl_y[id], tmpl_x[id]] \
                                for id in range(gt_y.shape[0])]).squeeze()
    ret_inner_2 = match_inner_product(raw_fea_list[ii], tmpl_feature)
    loss = ce_loss(ret_inner_2, gt_y, gt_x, nearby=nearby)
    return loss
    ...
    for epoch in range(config_train['num_epochs']):
        ...
        for time_load, index, (raw_img, crop_imgs, chosen_y, chosen_x, raw_y, raw_x) in tenum(dataloader):
            ...
            with torch.autograd.set_detect_anomaly(False):
                ...
                loss_5 = _tmp_loss_func(0, raw_fea_list, crop_fea_list,
                                        (raw_y, raw_x), (chosen_y, chosen_x))
                loss_4 = _tmp_loss_func(1, raw_fea_list, crop_fea_list,
                                        (raw_y, raw_x), (chosen_y, chosen_x), nearby=nearby)
                loss_3 = _tmp_loss_func(2, raw_fea_list, crop_fea_list,
                                        (raw_y, raw_x), (chosen_y, chosen_x), nearby=nearby)
                loss_2 = _tmp_loss_func(3, raw_fea_list, crop_fea_list,
                                        (raw_y, raw_x), (chosen_y, chosen_x), nearby=nearby)
                loss_1 = _tmp_loss_func(4, raw_fea_list, crop_fea_list,
                                        (raw_y, raw_x), (chosen_y, chosen_x), nearby=nearby)
