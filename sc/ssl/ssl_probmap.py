...
def test(logger, config):
    tester = Tester(logger, config, split="Test1+2", upsample='bilinear', collect_sim=False)
    learner = Learner(logger=logger, config=config)
    # learner.load(tfilename(config['base']['runs_dir'], 'ckpt_v', 'model_best.pth'))
    # pth = '/home1/quanquan/code/landmark/code/runs/ssl/ssl/collect_sim/ckpt_v/model_best.pth'
    pth = "/home1/quanquan/code/landmark/code/runs/ssl/ssl_hand/baseline/ckpt_v/model_best.pth"
    learner.load(pth)
    learner.cuda()
    learner.eval()
