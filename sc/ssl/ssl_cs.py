...
def _tmp_loss_func(ii, raw_fea_list, crp_fea_list, raw_loc, chosen_loc, nearby=None):
    scale = 2 ** (5-ii)
    raw_loc, chosen_loc = raw_loc // scale, chosen_loc // scale
    tmpl_feature = torch.stack([crp_fea_list[ii][[id], :, chosen_loc[id][0], chosen_loc[id][1]] \
...
class Learner(LearnerWrapper):
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        crp_imgs = data['crp_imgs']
        raw_loc  = data['raw_loc']
        chosen_loc = data['chosen_loc']

        raw_mask = data['raw_mask']
        raw_offset_w = data['raw_offset_w']
        raw_offset_h = data['raw_offset_h']
        crp_mask = data['crp_mask']
        crp_offset_w = data['crp_offset_w']
        crp_offset_h = data['raw_offset_h']

        raw_fea_list = self.net(raw_imgs)
        crp_fea_list = self.net_patch(crp_imgs)

        nearby = self.config['training']['nearby']
        loss_0 = _tmp_loss_func(0, raw_fea_list, crp_fea_list, raw_loc, chosen_loc)
        loss_1 = _tmp_loss_func(1, raw_fea_list, crp_fea_list, raw_loc, chosen_loc, nearby)
        loss_2 = _tmp_loss_func(2, raw_fea_list, crp_fea_list, raw_loc, chosen_loc, nearby)
        loss_3 = _tmp_loss_func(3, raw_fea_list, crp_fea_list, raw_loc, chosen_loc, nearby)
        loss_4 = _tmp_loss_func(4, raw_fea_list, crp_fea_list, raw_loc, chosen_loc, nearby)
