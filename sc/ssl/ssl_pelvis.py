...
from utils.tester.tester_pelvis_ssl import Tester
from datasets.pelvis.pelvis_ssl import Pelvis
...
class Learner(LearnerWrapper):
    ...
    def load(self):
        ...
        state_dict = '/home1/quanquan/code/landmark/code/runs/ssl/ssl_pelvis/debug3/ckpt/best_model_epoch_2180.pth'
...
def train(logger, config):
    ...
    dataset_train = Pelvis(config['dataset']['pth'], patch_size=config['training']['patch_size'])
...
def test(logger, config):
    ...
    exit(0)
    for i in range(47):
        res_dict = tester.test(learner, oneshot_id=i)
        logger.info(f"Results: {res_dict}")
        # break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="configs/ssl/ssl_pelvis.yaml")
