...
from datasets.ceph.ceph_heatmap_imaug import Cephalometric
...
class Learner(LearnerModule):
    def __init__(self, logger, config, *args, **kwargs):
        ...
        self.bcewithlogits_fn = torch.nn.BCEWithLogitsLoss(reduce=False)
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        loss = self.bcewithlogits_fn(heatmap_pred, heatmap)
        loss = reduce(loss, "b c h w -> b c", reduction='sum')
        if self.config['special']['use_adap_loss']:
            ...
            loss2 = loss.mean().item()
            loss = loss * ww / aug_percent
            loss = loss.mean()
            return {'loss': loss, "real_loss": loss2}
        else:
            loss = loss.mean()
            if torch.isnan(loss):
...
def train(logger, config):
    ...
    tester = Tester(logger=None, config=config, mode="Test1+2", get_mre_per_lm=True)
...
def test(logger, config):
    ...
    csvlogger = CSVLogger("./tmp/", 'adaploss.csv')
