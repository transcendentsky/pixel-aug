...
def train(logger, config):
    ...
    tester_subtest = Tester(logger=None, config=config, split="Test1+2", get_mre_per_lm=True)
...
def test(logger, config):
    model = Learner(logger=None, config=config)
    tester = Tester(logger=logger, config=config, split="Test1+2", get_mre_per_lm=True)
    ...
    model.eval()
    test_d = tester.test(model, epoch=44)
