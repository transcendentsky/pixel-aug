...
class Learner(LearnerModule):
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        if self.config['special']['use_adap_loss']:
            aug_percent = np.array(self.config['special']['aug_percent'])
            aug_percent = repeat(aug_percent, "c -> b c", b=img.shape[0])
            aug_percent = torch.Tensor(aug_percent).cuda()
            loss2 = logic_loss.mean().item()
            # import ipdb; ipdb.set_trace()
            ww = ww / aug_percent

            logic_loss = logic_loss * ww
            logic_loss = logic_loss.mean() / 10000.0

            regression_loss_x = self.loss_regression_fn(regression_x, offset_x, mask, ww)
            regression_loss_y = self.loss_regression_fn(regression_y, offset_y, mask, ww)

            loss = regression_loss_x + regression_loss_y + logic_loss * self.config['training']['lambda']
            # import ipdb; ipdb.set_trace()

            return {'loss': loss, "logic_loss": logic_loss, "regloss_x": regression_loss_x, "regloss_y":regression_loss_y, "real_logic_loss": loss2}
        else:
...
def train(logger, config):
    ...
    tester_subtest = Tester(logger=None, config=config, split="Test1+2")
    monitor = Monitor(key='mre', mode='dec')
    # dataset = Cephalometric(config['dataset']['pth'], return_offset=True)
    dataset = Cephalometric(config['dataset']['pth'], adap_mode="adap0", return_offset=True, 
                            cj_brightness=config['special']['cj_brightness'], 
                            cj_contrast=config['special']['cj_contrast'],
                            cached_params_path="./cache/optimal_brct_2.npy")
