...
def embed_feas(raw_fea_list, raw_loc):
    flist = []
    for ii in range(5):
        scale = 2 ** (5-ii)
        raw_loc, chosen_loc = raw_loc // scale
        tmpl_feature = torch.stack([raw_fea_list[ii][[id], :, raw_loc[id][0], raw_loc[id][1]] \
                                    for id in range(raw_loc.shape[0])]).squeeze()
        flist.append(tmpl_feature)
    return torch.stack(flist, dim=0)
...
class Learner(LearnerWrapper):
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        embeded_feas = embed_feas(raw_fea_list, raw_loc)
