...
from . import sampler_vaal as sampler
import copy
...
class Solver:
    ...
    def train(self, querry_dataloader, val_dataloader, task_model, vae, discriminator, unlabeled_dataloader):
        print("debug! Start Solver training")
        ...
        for iter_count in tqdm(range(self.args.train_iterations)):
            ...
            # task_loss = task_model.loss(preds, labels)
            ...
            for count in range(self.args.num_vae_steps):
                # assert labeled_imgs.size()[1:] == (3,224,224), f"Got labeled_imgs shape: {labeled_imgs.shape}"
                recon, z, mu, logvar = vae(labeled_imgs)
                unsup_loss = self.vae_loss(labeled_imgs, recon, mu, logvar, self.args.beta)
                # assert unlabeled_imgs.size()[1:] == (3,224,224), f"Got unlabeled_imgs shape: {unlabeled_imgs.shape}"
                ...
                if lab_real_preds.any() > 1. or lab_real_preds.any() < 0.:
                    print("lab_read_preds:", lab_real_preds)
                if unlab_real_preds.any() > 1. or unlab_real_preds.any() < 0.:
                    print("unlab_read_preds:", unlab_real_preds)

                dsc_loss = self.bce_loss(labeled_preds.squeeze().clamp(0, 1), lab_real_preds.squeeze()) + \
                           self.bce_loss(unlabeled_preds.squeeze().clamp(0, 1), unlab_real_preds.squeeze())
