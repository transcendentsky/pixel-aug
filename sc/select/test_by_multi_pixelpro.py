...
def review_multi(logger, config, args):
    ...
    if not args.review:
        pred_record_pth = '/home1/quanquan/code/landmark/code/runs/select/test_by_multi_pixelpro/debug/pred_record_list.npy'
    ...
    ####
    # mre_rank = np.argsort(mre_list)
    conf_rank = np.argsort(conf_list)
    # print(conf_list)
    # print(conf_rank)
    conf_list = np.array(conf_list)[conf_rank]
    mre_list = np.array(mre_list)[conf_rank]
    ids_list = np.array(ids_list)[conf_rank]
    best_ids = ids_list[-1]
    ...
    # import ipdb; ipdb.set_trace()
    mre_rank = np.argsort(mre_list)
    # r1 = np.where(mre_rank == (len(mre_rank)-1))[0][0]
    # r2 = np.where(mre_rank == (len(mre_rank)-2))[0][0]
    # r3 = np.where(mre_rank == (len(mre_rank)-3))[0][0]
    rlist = [np.where(mre_rank == (len(mre_rank) - i - 1))[0][0] for i in range(len(mre_rank))]
    rlist = np.array(rlist) / len(mre_rank)
    logger.info(f"r1:{rlist} total:{len(mre_list)}")
    best_ids = ids_list[-1]
    logger.info(f"conf: {conf_list[0]}, {conf_list[-1]}, {best_ids}")
    logger.info(f"mre: {mre_list[:5]}, {mre_list[-5:]}")
    logger.info(f"mre_avg: {np.mean(mre_list)}")
    logger.info(f"mre_rank: {mre_rank[0]}, {mre_rank[-1]}")
    print("Test_specific_ids")
    # print(mre_list)

    draw_scatter(conf_list, mre_list, config['base']['runs_dir'] + f"/sort_n{num_ref}.png", xlabel="sim",
                 ylabel="mre")
    # exit(0)
    ...
    # conf_list = np.load(max_list_pth)
    #
    # mre_list_pth = config['mre_list_pth'] = tfilename(config['base']['runs_dir'], f"mre_n{num_ref}_list.npy")
    # mre_list = np.load(mre_list_pth)
    #
    # draw_scatter(conf_list, mre_list, config['base']['runs_dir'] + f"/sort_sort_n{num_ref}_{tag}.pdf", xlabel="sim",
    #              ylabel="mre")
    # draw_scatter(conf_list, mre_list, config['base']['runs_dir'] + f"sort_sort_n{num_ref}_{tag}.png", xlabel="sim",
    #              ylabel="mre")
    # print("Saved :", config['base']['runs_dir'] + f"sort_sort_{tag}.png")
    # dump_yaml(logger, config)
    #
    # conf_sort = np.argsort(conf_list)[::-1]
    # mre_sort = np.argsort(mre_list)
    # print("Max sim: ", mre_list[mre_sort[:3]], conf_list[mre_sort[:3]])
    # print("Max sim: ", mre_list[conf_sort[:3]], conf_list[conf_sort[:3]])
...
if __name__ == '__main__':
    ...
    # nlist = [1,2,3,4,5,15,45]
    nlist = [1,2,3]
    for n in nlist:
        logger.info(f"Change n to {n}")
        config['base']['ref'] = n
        review_multi(logger, config, args)
    # ckpt = '/home1/quanquan/code/landmark/code/runs/ssl/pixel_contra/debugx/ckpt/model_latest.pth'
