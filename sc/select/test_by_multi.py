...
def preprocessing(logger, config):
    ...
    print(f"debug train2.py non local={config['special']['non_local']}")
    ...
    new_dict = {k[4:]:v for k, v in ckpt.items()}
...
class SimpleEvaler(object):
    ...
    def test_multi(self, ids, valid_percent=1.0, draw=False):
        # For arbitary ids
        ...
        for i, data in enumerate(self.dataloader_1):
            ...
            preds = res[i, :, :].transpose((1,0))
            detailed_error = self.evaluater.record(preds, landmark_list)

            if draw:
                image_pred = visualize(img, preds, landmark_list, num=19)
                image_pred.save(tfilename(self.config['base']['runs_dir'], f'visuals', f'pred_{i+1}.png'))
                detailed_error = np.array(detailed_error).mean()
                print(f"Id: {i}, Error: {detailed_error}")

        mre_dict = self.evaluater.cal_metrics()
        return mre_dict, max_mean_conf, max_conf
...
def review_multi(logger, config):
    ...
    if not args.review:
        ...
        while True:
            ...
            mre_dict, conf, maxconf = eval.test_multi(ids, valid_percent=valid_percent, draw=False)
            mre = mre_dict['mre']
            ...
            # print(rdict)
            # print(radius_res_list)
            # print(len(radius_res_list[0]))
            # import ipdb; ipdb.set_trace()
            print("debug mre=", mre)
            if mre < 3:
                mre_dict, conf, maxconf = eval.test_multi(ids, valid_percent=valid_percent, draw=True)
                import ipdb; ipdb.set_trace()
