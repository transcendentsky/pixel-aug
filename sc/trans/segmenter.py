...
from tutils import tfilename, trans_args, trans_init, dump_yaml, tdir, save_script, CSVLogger
...
class FunctionManager(object):
    ...
    def test(self, logger, config, args):
        csvlogger = CSVLogger(config['base']['runs_dir'], name="test_record.csv")
        model = Learner(config, logger)
        epoch = args.epoch
        pth = tfilename(config['base']['runs_dir'], f"model_epoch_{epoch}.pth")
        model.load(pth)
        model.cuda()
        tester_train = Tester(logger, config, mode="Train")
        tester_test = Tester(logger, config, mode="Test1+2")
        logger.info(f"Dataset Training")
        _d = tester_train.test(model)
        csvlogger.record(_d)
        logger.info(_d)
        logger.info(f"Dataset Test 1+2")
        _d = tester_test.test(model)
        csvlogger.record(_d)
        logger.info(_d)
