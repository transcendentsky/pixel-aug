"""
    seperate entropy map with hard entropy thresholds

    gaussian prob map + entropy map
"""
import torch
...
def train(logger, config):
    ...
    prob_map = np.zeros((384,384))
    frac = 1
    border = int(384 * frac)
    prob_map[:, :border] = 1
    cv2.imwrite(f"./tmp/ssl_probmap5_block_probmap{frac}.jpg", (prob_map * 255).astype(np.uint8))
    dataset_train = Cephalometric(config['dataset']['pth'], patch_size=config['special']['patch_size'],
                             prob_map=prob_map, entr_map_dir=config['dataset']['entr'],
                             mode="Train", retfunc=2, hard_select=True)
    entr_per, select_per = dataset_train.entr_map_from_image(threshold=1.8, threshold2=0, ratio=config['special']['prob_ratio'])
    # logger.info("Threshold: {}")
    logger.info(f"Entr_per: {entr_per}, select_per: {select_per}")
    # 1.75 -> 0.7
    # import ipdb; ipdb.set_trace()
