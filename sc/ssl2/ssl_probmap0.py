"""
    Randomly generate prob map!
    For paper writing and comparison
"""
import torch
...
def train(logger, config):
    ...
    # landmark_list = np.random.rand(19,2)*383
    landmark_list = None
    prob_map = np.zeros((384,384))
    frac = 1
    border = int(384 * frac)
    prob_map[:, :border] = 1
    cv2.imwrite(f"./tmp/block_probmap{frac}.jpg", (prob_map * 255).astype(np.uint8))
    # torchvision_save(torch.Tensor(prob_map), f"./tmp/block_probmap{frac}.jpg")
    dataset_train = Cephalometric(config['dataset']['pth'], patch_size=config['special']['patch_size'],
                                  ...
                                  sharpness=0.4, prob_map=prob_map)
