...
EX_CONFIG = {
    ...
    "special": {
        "cj_brightness": 0.8, # 0.15
        "cj_contrast": 0.6, # 0.25
        "cj_saturation": 0., # 0.
        "cj_hue": 0., # 0
        "use_adap_aug": True,
        "weighted_t": 0.3, # temperature
...
# def _tmp_get_weight(gt_values, epoch=0):
#     t = EX_CONFIG['special']['weighted_t']
#     v = (-gt_values * (epoch) * t).exp()
#     w = v / v.sum() * gt_values.shape[0]
#     return w
...
def train(logger, config):
    ...
    dataset_train.entr_map_ushape2(temperature=0.3)
