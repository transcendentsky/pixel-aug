...
EX_CONFIG = {
    "dataset":{
        'entr': '/home1/quanquan/datasets/hand/hand/entr3/train/',
        'prob': '/home1/quanquan/datasets/hand/hand/prob_size10/train/',
    },
    "special": {
...
def train(logger, config):
    from datasets.hand.hand_basic import HandXray as TestHandXray
    ...
    dataset_train = HandXray(config['dataset']['pth'],
                             ...
                             multi_output=False,
                             prob_mode="entr",)
    dataset_train.entr_map_from_image(temperature=config['special']['entr_t'], inverse=config['special']['inverse'])
    # testset = TestHandXray(config['dataset']['pth'], ret_mode="no_process")
    # all_landmarks = []
    # for i in range(len(testset)):
    #     _, landmark_list, _, _ = testset._get_data(i)
    #     all_landmarks.append(landmark_list)
    # dataset_train.save_ref_landmarks(all_landmarks)
    # dataset_train.prob_map_for_all(all_landmarks, kernel_size=config['special']['probmap_ks'])
...
def test(logger, config):
    ...
    # pth = "/home1/quanquan/code/landmark/code/runs/ssl/ssl/ps64/ckpt_v/model_best.pth"
    # learner.load(pth)
    pth = None
    logger.info(pth)
    learner.cuda()
