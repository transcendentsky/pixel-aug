"""
    Plan:
        select different augmentations to reduce sim ?
        ? 
"""
...
from datasets.ceph.ceph_ssl_aug import Cephalometric
...
def train(logger, config):
    ...
    no_prob = config['special']['noprob']
    id_oneshot = 114
    tester = Tester(logger, config, default_oneshot_id=id_oneshot, collect_sim=True)
...
def test(logger, config):
    from utils.tester.tester_ssl import Tester as Tester_1
    from utils.tester.tester_ssl_ensemble import Tester as Tester_2
    from utils.tester.tester_ssl_ensemble2 import Tester as Tester_3

    split = "Test1+2" # Test1+2
    tester1 = Tester_1(logger, config, split=split)
    tester2 = Tester_2(logger, config, split=split)
    tester3 = Tester_3(logger, config, split=split)
    learner = Learner(logger=logger, config=config)
    ...
    for id_oneshot in ids:
        # res = tester1.test(learner, oneshot_id=id_oneshot)
        # logger.info("Standard")
        # logger.info(res)
        res = tester2.test(learner, oneshot_id=id_oneshot)
        logger.info("Emsemble 1")
        logger.info(res)
        res = tester3.test(learner, oneshot_id=id_oneshot)
        logger.info("Emsemble 2")
        logger.info(res)
        exit()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="configs/ssl2/ssl_probmap.yaml")
    parser.add_argument('--noprob', action='store_true')
