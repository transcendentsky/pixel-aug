...
def test(logger, config):
    ...
    tester = Tester(logger, config, default_oneshot_id=template_oneshot_id,
                    collect_sim=False, split="Train", upsample="bilinear",
                    get_mre_per_lm=True)
    learner = Learner(logger, config)
    learner.load("/home1/quanquan/code/landmark/code/runs/ssl/ssl_probmap7/gaussian_ps64_pseudo/ckpt_v/model_best.pth")
    learner.cuda()
    learner.eval()
