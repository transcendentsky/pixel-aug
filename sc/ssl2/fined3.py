"""
    Compared with fined_fea.py, this script use interpolated cos sim map instead of scaled concat features

    fined2 used self-made interpolation , and  inconsist with eval by upscale sim maps
    fined3 upscale sim maps in training too.
"""
...
def _tmp_loss_func(ii, raw_fea_list, crop_fea_list, raw_loc, chosen_loc, nearby=None):
    ...
    # import ipdb; ipdb.set_trace()
    product = torch.nn.functional.upsample(product.unsqueeze(1),
                                           scale_factor=2 ** (5 - ii), mode="bilinear").squeeze()
    # gt_values = [interpolate_gt_value(product[i], raw_loc[i, 0], raw_loc[i, 1]) for i in range(product.shape[0])]
    gt_values = [product[i, raw_loc[i, 0], raw_loc[i, 1]] for i in range(product.shape[0])]
    raw_loc = raw_loc // scale
