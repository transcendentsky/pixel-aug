...
from models.transformer.segmenter import Segmenter, Segmenter2
import os
...
class Learner(LearnerModule):
    def __init__(self, logger, config, *args, **kwargs):
        ...
        self.net = Segmenter2(config, up_scale=True)
    ...
    def training_step(self, data, batch_idx, **kwargs):
        ...
        heatmap_pred = self.net(img, sigmoid=False)
...
def train(logger, config):
    ...
    tester = Tester(logger=None, config=config, mode="subtest")
...
def test(logger, config):
    ...
    # test_d = tester.test2(model, draw=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", default='configs/baseline/segmenter.yaml', help="name of the run")
