...
from utils.tester.tester_pelvis import Tester
...
class Learner(LearnerModule):
    ...
    def training_step(self, data, batch_idx, **kwargs):
        img = data['img']
        mask = data['mask']
        offset_w = data['offset_w']
        offset_h = data['offset_h']

        heatmap, regression_h, regression_w = self.forward(img)

        logic_loss = self.loss_logic_fn(heatmap, mask)
        regression_loss_w = self.loss_regression_fn(regression_w, offset_w, mask)
        regression_loss_h = self.loss_regression_fn(regression_h, offset_h, mask)

        loss = regression_loss_w + regression_loss_h + logic_loss * self.config['training']['lambda']

        if torch.isnan(loss):
            print("debug: ", img.shape, mask.shape, heatmap.shape, loss)
        return {'loss': loss, "logic_loss": logic_loss, "regloss_w": regression_loss_w, "regloss_h":regression_loss_h}
    ...
    def load(self):
        state_dict = '/home1/quanquan/code/landmark/code/runs/baseline/baseline_pelvis/debug3/ckpt/best_model_epoch_300.pth'
...
def train(logger, config, args):
    ...
    dataset = Pelvis(config['dataset']['pth'], datanum=args.datanum)
...
def test(logger, config, args):
    ...
    learner.load()
    learner.cuda()
    res_dict = tester.test(learner, draw=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", default='configs/baseline/baseline_pelvis.yaml', help="name of the run")
