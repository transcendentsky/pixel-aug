...
def preprocessing(logger, config):
    ...
    net = UNet_Pretrained(3, non_local=config['non_local'], emb_len=16)
    ...
    np.save(tfilename(config['runs_dir'], f"mre_list.npy"), mre_list)
    np.save(tfilename(config['runs_dir'], f"ids_list.npy"), ids_list)
    np.save(tfilename(config['runs_dir'], f"pred_record_list.npy"), record_list)
...
class SimpleEvaler(object):
    ...
    def test_multi(self, ids, valid_percent=1.0):
        ...
        return mre, max_mean_conf, rdict, max_conf
