"""
    up scale global fea maps for more accurate features

    used with ssl2/fined_fea.py
"""
...
def feature_syn(feature_map, h, w):
    ...
    h_floor, h_ceil = torch.clamp(h_floor, 0, feature_map.shape[1]-1).long(), torch.clamp(h_ceil, 0, feature_map.shape[1]-1).long()
    w_floor, w_ceil, w_ratio = torch.floor(w), torch.ceil(w), torch.ceil(w) - w
    w_floor, w_ceil = torch.clamp(w_floor, 0, feature_map.shape[2]-1).long(), torch.clamp(w_ceil, 0, feature_map.shape[2]-1).long()
    # import ipdb; ipdb.set_trace()
...
class Tester(object):
    def __init__(self,
        ...
        print("Warning! using tester_fined! ")
    ...
    def test_func(self, net, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False, draw=False, collect_details=False):
        ...
        for id_depth in range(6):
            ...
            for id_mark, landmark in enumerate(landmarks):
                ...
                mark_feature = feature_syn(features_tmp[id_depth][0, :, :, :].cpu(), tmpl_y, tmpl_x)
        ...
        for ID, data in enumerate(self.dataloader_1):
            ...
            for id_mark in range(one_shot_feature.shape[0]):
                ...
                for id_depth in range(5):
                    assert len(features[id_depth].shape) == 4, f"Got {features[id_depth].shape}"
                    upscaled_feature = torch.nn.functional.upsample(features[id_depth], scale_factor=2**(5-id_depth), mode="bilinear")
                    cos_similarity = match_cos(upscaled_feature.squeeze(), \
                                               feature_list[id_depth][id_mark])                   # import ipdb;ipdb.set_trace()
