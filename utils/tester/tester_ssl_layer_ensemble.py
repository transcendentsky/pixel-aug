...
def extract_vectors_via_points(feature_map, landmarks):
    """
    Used for extracting template vectors
    :param feature_map: [1, c, h w]
    :param landmarks:   [[w, h], [w, h], ...]
    :return:
    """
    feature_list = dict()
    for id_depth in range(6):
        ...
        for id_mark, landmark in enumerate(landmarks):
            ...
            mark_feature = feature_map[id_depth][0, :, tmpl_y, tmpl_x]
...
class Tester(object):
    ...
    def test_func_collect_sim(self, nets, *args, **kwargs):
        res1 = self.test_func(nets, collect_details=True, *args, **kwargs)
    ...
    def test_func(self, nets, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False,
                  draw=False, collect_details=False,
                  gates=None):

        gates_index = [0] * len(gates)
        assert len(gates) == 5, f"Got {gates}"
        for i, (name, model) in enumerate(nets.items()):
            for j, g in enumerate(gates):
                if g == name:
                    gates_index[j] = i
        # print("Debug, ", gates_index, gates, nets.keys())

        nets = [net.eval() for k, net in nets.items()]
        ...
        print('Using models: ')
        for i in range(5):
            print(f"    layer {i} : model {gates[i]}, gate_index {gates_index[i]}")
        ...
        feature_tmp_list = [net(image.unsqueeze(0)) for net in nets]
        # features_tmp = net(image.unsqueeze(0))

        # Change the original template vector list to the ensemble vectors of 5 models
        vectors_of_5models_list = [extract_vectors_via_points(feature_tmp, landmarks) for feature_tmp in feature_tmp_list]
        feature_list = [vectors_of_5models_list[gates_index[i]][i] for i in range(5)]

        num_landmark = 19
        assert num_landmark == len(landmarks), f"Check num_landmark, it should be {len(landmarks)}"
        ...
        for ID, data in enumerate(self.dataloader_1):
            ...
            # import ipdb; ipdb.set_trace()
            img = data['img'].cuda()
            features_all_models = [net(img) for net in nets]
            features = [features_all_models[gates_index[i]][i] for i in range(5)]
            ...
            for id_mark in range(num_landmark):
                ...
                for id_depth in range(5):
                    ...
                    cos_similarity = torch.nn.functional.upsample( \
                        ...
                        scale_factor=2 ** (5 - id_depth), mode=self.upsample).squeeze()
                    # import ipdb;ipdb.set_trace()
        ...
        if collect_details:
            ...
            return {**mre, **mre_all, "oneshot_id": oneshot_id, "split": self.split, "upsample": self.upsample}
