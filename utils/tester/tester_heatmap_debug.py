...
class Tester(object):
    ...
    def test_func(self, net, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False, draw=False, collect_details=False):
        ...
        image = image.cuda()
        features_tmp = net.get_features(image.unsqueeze(0))

        # Depth
        tmp = list()
        for id_mark, landmark in enumerate(landmarks):
            tmpl_y, tmpl_x = landmark[1], landmark[0]
            mark_feature = features_tmp[0, :, tmpl_y, tmpl_x]
        ...
        for ID, data in enumerate(self.dataloader_1):
            ...
            for id_mark in range(one_shot_feature.shape[0]):
                ...
                cos_similarity = match_cos(features.squeeze(), \
                                           one_shot_feature[id_mark])
        ...
        if collect_details:
            return {**mre, **mre_all, "oneshot_id": oneshot_id, "split": self.split,
                    ...
                    "mean_sim": mean_list_list,
                    "near_sim": near_list_list}
        else:
            return {**mre, **mre_all, "oneshot_id": oneshot_id, "split": self.split}
