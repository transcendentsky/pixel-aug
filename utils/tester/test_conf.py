...
class Tester(object):
    ...
    def test(self, net, epoch=None, dump_label=False, tag=None, oneshot_id=126, draw=True):
        ...
        for i in range(1):
            one_shot_loader = Test_Cephalometric(pathDataset=self.config['dataset_pth'], \
            ...
            for img, landmark_list in self.dataloader_1:
                ...
                for id_mark in range(one_shot_feature.shape[0]):
                    ...
                    if draw:
                        gray_to_PIL2(debug.cpu().detach(), pred_landmark, a_landmark).save(\
                            tfilename(config['runs_dir'], 'visuals', str(ID), f'{id_mark+1}_debug_w_gt.jpg'))
                ...
                if draw:
                    ...
                    image_pred.save(tfilename(config['runs_dir'], 'visuals', str(ID), 'pred.png'))

                if dump_label:
                    inference_marks = {id:[int(preds[1][id]), \
                        int(preds[0][id])] for id in range(19)}
                    dir_pth = tfilename(config['runs_dir'], 'pseudo-labels_init')
