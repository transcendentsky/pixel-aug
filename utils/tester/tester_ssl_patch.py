...
class Tester(object):
    ...
    def test_func(self, net, net_patch=None, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False, draw=False, collect_details=False):
        # if net is not None:
        #     self.model = net
        ...
        # image, patch_landmarks, template_patches, landmarks = one_shot_loader.__getitem__(oneshot_id)
        image, template_patches, landmarks, im_name = data['raw_imgs'], data['crop_imgs'], data['landmark_list'], data['chosen_loc']
        feature_list = list()
        if rank != 'cuda':
            image = image.to(rank)
        else:
            image = image.cuda()
        features_tmp = net_patch(template_patches)
