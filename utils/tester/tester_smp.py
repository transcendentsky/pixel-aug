...
class Tester(object):
    def __init__(self,
        ...
        dataset_1 = Test_Cephalometric(config['dataset']['pth'], mode=split, size=size)
        # print("Test")
    ...
    def test_func(self, net, epoch=None, rank='cuda', oneshot_id=-1, dump_label=False, draw=False, collect_details=False):
        ...
        one_shot_loader = Test_Cephalometric(pathDataset=self.config['dataset']['pth'], mode='Train', size=self.size)
        ...
        # assert image.shape[-1] == 640 and image.shape[-2] == 800, f"Got {image.shape}"
        ...
        for ID, data in enumerate(self.dataloader_1):
            ...
            if rank != 'cuda':
                ...
                img = data['img'].cuda()
            # assert img.shape[-1] == 640 and img.shape[-2] == 800, f"Got {img.shape}"
            features = net(img)
            ...
            for id_mark in range(one_shot_feature.shape[0]):
                ...
                pred_landmarks_y.append(chosen_landmark // self.size[1])
                pred_landmarks_x.append(chosen_landmark % self.size[1])
                pred_lm.append([chosen_landmark // self.size[1], chosen_landmark % self.size[1]])

                if collect_details:
                    # cos_list_list.append(cos_lists.detach().cpu().numpy())
                    final_cos = final_cos.clamp(0, 1)
                    max_sim = final_cos[chosen_landmark // self.size[1], chosen_landmark % self.size[1]]
                    max_list.append(max_sim.detach().cpu().numpy())
                    # for i in range(5):
                    #     print(f"layer {i}: target sim {cos_lists[i][chosen_landmark // self.size[1], chosen_landmark % self.size[0]]} , max sim {cos_lists[i].max().item()}")
                    ...
                    if self.collect_near:
                        ...
                        for a_loc_list in loc_list:
                            ...
                            for i, a_loc in enumerate(a_loc_list):
                                h, w = chosen_landmark // self.size[1] + a_loc[0], chosen_landmark % self.size[1] + a_loc[1]
                    ...
                    for layer_i, cos in enumerate(cos_lists[:-1]):
                        cos = cos.clamp(0, 1)
                        value_of_pred = cos[chosen_landmark // self.size[1], chosen_landmark % self.size[1]].detach().cpu().numpy()
                ...
                if draw:
                    ...
                    a_landmark = landmark_list[id_mark]
                    pred_landmark = (chosen_landmark % self.size[1], chosen_landmark // self.size[0])
